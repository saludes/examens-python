...
Donada la matriu
\[
   A=\begin{pmatrix}{*{3}r} 3 & 2 & -2\\ 1 & 4 & -2\\ 2 & 2 & -1\end{pmatrix}\,,
\]
\begin{apartats}
...
\[
  \left.\aligned 2 x - y + 2 z &= 4 \\ m x + y - 3 z &= -8 \\ 4 x + 4 y - 2 z &= m + 12 \endaligned\;\right\}\,,
\]
\begin{apartats}
...
\begin{tabular}{|L{3cm}{1.5cm}|L{7cm}{1.5cm}|}
\hline
$m=-4$ & $\left.\aligned x &= \frac{- z + 4}{2} \\ y &= z \endaligned\;\right\}$\\
\hline
\end{tabular}
\end{center}
\end{solucio}
...
\begin{pmatrix}{*{1}r} 8\\ 4\\ 2\end{pmatrix} + 
\begin{pmatrix}{*{3}r} 0 & 0 & 1\\ -1 & 0 & 0\\ 0 & -1 & 0\end{pmatrix}
\begin{pmatrix}{c} x \\ y \\ z\end{pmatrix}$ \\
\hline
\multicolumn{1}{|C{10cm}{0.8cm}|}
{$\psi=180^\circ$\hspace{1.7cm}$\theta=90^\circ$\hspace{1.7cm}$\phi=90^\circ$} \\
\hline
\end{tabular}
\end{center}
\end{solucio}
...
\[
  \frac{x - 2}{3} = y + 1 = z + 2\qquad\text{i}\qquad \left.\aligned - 3 x + 2 y + z &= -11 \\ - x - 2 y + 3 z &= -1 \endaligned\;\right\}\,.
\]
\begin{apartats}
...
\[
  3 x^{2} + 3 y^{2} - 2 x y + 8 x + 8 y -16 = 0
\]
\begin{apartats}
...
Canonica(-15,11,-12,8);
Ellipse((-2,-2),(1,1),16,8,x=13,y=10);
clip((-15,-12)--(11,-12)--(11,8)--(-15,8)--cycle);
\end{asy}
\end{center}
\caption{Representació gràfica de la cònica}
\end{figure}
...
\hline
$\cal R'=\left\{(-2,-2);\deufrac{1}{\sqrt{2}}(1,1),\deufrac{1}{\sqrt{2}}(-1,1)\right\}$\hspace{2cm} $\displaystyle \frac{x'^2}{16} + \frac{y'^2}{8} = 1$  \hspace{2cm} El·lipse\\
\hline
\end{tabular}
\end{center}
\end{solucio}

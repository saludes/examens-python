...
Donada la matriu
\[
   A=\begin{pmatrix}{*{3}r} 3 & 2 & -2\\ 4 & 4 & -5\\ 2 & 2 & -1\end{pmatrix}\,,
\]
\begin{apartats}
...
\[
  \left.\aligned - 3 x - 3 y + m z &= -9 \\ - x - y + z &= -3 \\ x - y + 3 z &= - m \endaligned\;\right\}\,,
\]
\begin{apartats}
...
\begin{tabular}{|L{3cm}{1.5cm}|L{7cm}{1.5cm}|}
\hline
$m=3$ & $\left.\aligned x &= - z \\ y &= 2 z + 3 \endaligned\;\right\}$\\
\hline
\end{tabular}
\end{center}
\end{solucio}
...
\[
  x - 1 = \frac{y + 2}{-1} = \frac{z + 2}{-1}\,.
\]
\begin{apartats}
\item Quina és la representació en la referència canònica del moviment helicoidal que consisteix en una rotacio d'angle $240^\circ$ al voltant de la recta $R$ seguida d'una translació de $(-3,3,3)$.
...
\begin{pmatrix}{*{1}r} -4\\ 3\\ 2\end{pmatrix} + 
...
\[
  x + 3 = \frac{y - 3}{-1} = \frac{z + 2}{-3}\qquad\text{i}\qquad \left.\aligned - 5 x - 6 y + 4 z &= -16 \\ x + 6 y + 4 z &= 32 \endaligned\;\right\}\,.
\]
\begin{apartats}
...
\hline
$d=\displaystyle \frac{5 \sqrt{66}}{22}$ & $4 x + 7 y - z = 11$ \\
\hline
\end{tabular}
\end{center}
\end{solucio}
...
\[
  3 x^{2} + 3 y^{2} + 10 x y - 2 x - 14 y -13 = 0
\]
\begin{apartats}
...
Canonica(-11,15,-11,9);
Hiperbola((2,-1),(1,1),1,4,x=10,y=10);
...
\hline
$\cal R'=\left\{(2,-1);\deufrac{1}{\sqrt{2}}(1,1),\deufrac{1}{\sqrt{2}}(-1,1)\right\}$\hspace{2cm} $\displaystyle x'^2 - \frac{y'^2}{4} = 1$  \hspace{2cm} Hipèrbola\\
\hline
\end{tabular}
\end{center}
\end{solucio}
